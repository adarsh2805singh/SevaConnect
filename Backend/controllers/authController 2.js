...
exports.registerVolunteer = async (req, res) => {
  try {
    const { email, password, name, phone, skills, availability } = req.body;
...
module.exports = exports;
  try {
    ...
    // Create new volunteer
    const newVolunteer = new User(email, password, name, phone, 'volunteer', {
      skills: skills || [],
      availability: availability || []
    });

    // Hash password
    await newVolunteer.hashPassword();

    // Save to mock database
    usersDatabase.push(newVolunteer);
...
exports.loginVolunteer = async (req, res) => {
  try {
    ...
    // Find user
    const user = usersDatabase.find(u => u.email === email && u.role === 'volunteer');
    if (!user) {
      return res.status(401).json({
        success: false,
        message: 'Invalid email or password'
...
exports.registerNGO = async (req, res) => {
  try {
    const { email, password, name, phone, organizationName, sector, description } = req.body;

    // Validation
    if (!email || !password || !name || !phone || !organizationName || !sector) {
      return res.status(400).json({
        success: false,
        message: 'Email, password, name, phone, organization name, and sector are required'
      });
    }
    ...
    // Create new NGO
    const newNGO = new User(email, password, name, phone, 'ngo', {
      organizationName,
      sector,
    ...
    // Hash password
    await newNGO.hashPassword();

    // Save to mock database
    usersDatabase.push(newNGO);
...
exports.loginNGO = async (req, res) => {
  try {
    ...
    // Find user
    const user = usersDatabase.find(u => u.email === email && u.role === 'ngo');
    if (!user) {
      return res.status(401).json({
        success: false,
        message: 'Invalid email or password'
    ...
    if (user.status !== 'approved') {
      return res.status(403).json({
        success: false,
        message: `NGO is currently ${user.status}. Please wait for admin approval.`
      });
    }
...
exports.getAllUsers = (req, res) => {
  try {
    const users = usersDatabase.map(u => u.toJSON());
    res.status(200).json({
      success: true,
      message: 'Users retrieved successfully',
      data: users,
